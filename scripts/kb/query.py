...
def _check_model_compatibility(indexer: Any, embedder: Any) -> None:
    ...
    if len(real) > 1:
        raise RuntimeError(
            f"query: DB contains mixed embed_models {real!r} — contaminated, "
            f"refusing to query. Rebuild from sidebars: "
...
def query(
    ...
    # B20: 用 _bm25_text 优先取 context（信息最丰富），回退 description/title。
    from rank_bm25 import BM25Okapi

    corpus = [_tokenize(_bm25_text(c)) for c in candidates]
...
def _rerank(question: str, results: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    Raises ImportError if flashrank isn't installed — Rule 12 (失败必须显性化):
    the caller explicitly asked for rerank, so silent degradation would
    mislead them into treating un-reranked results as reranked.
    """
    if not results:
        return results
    try:
        ...
        raise ImportError(
            "flashrank is required for rerank=True but not installed. "
            "Install with `pip install flashrank` or call query with rerank=False."
        ) from e
    ranker = RankModel()
    passages = [
