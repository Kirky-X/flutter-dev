"""Sidebar markdown parser for Flutter docs.

Parses Flutter sidebar `.md` files into doc records. Each line of the form

    ## N. [title](url)
    ### N.N. [title](url)
    #### N.N.N. [title](url)
    ### N.N. [`package_name`](url)   (with backticks for API sidebar)

( 2-4 hashes, a numbered heading, then a markdown link with optional backticks
around the title ) is one doc record.

The regex: `^#{2,4}\\s+[\\d.]+\\s+\\[`?([^\\]]+?)`?\\]\\(([^)]+)\\)`.
Lines whose heading has plain text only (no `[title](url)` link) are NOT
doc records — they are category headers.
"""
...
# 2-4 hashes + numbered heading + optional backticks around title + markdown link.
# The `? after `\`` makes backticks optional (docs/ai-docs sidebars have no
# backticks; api sidebar uses [`pkg`](url) format).
LINE_RE = re.compile(r"^#{2,4}\s+[\d.]+\s+\[`?([^\]]+?)`?\]\(([^)]+)\)")

# Map sidebar filename -> doc_type. 3 Flutter sidebars.
SIDEBAR_FILE_MAP: dict[str, str] = {
    "flutter-docs.md": "docs",
    "flutter-api.md": "api",
    "flutter-ai-docs.md": "ai-docs",
}
...
def _make_content_hash(
    ...
    # \x1f (ASCII Unit Separator) delimits fields so ("ab","cd") and ("a","bcd")
    # can't collide — fixing the field-boundary collision class (B7).
    raw = (
        f"{title}\x1f{url}\x1f{doc_type}\x1f{description}\x1f{context}\x1f"
        f"|links:{','.join(sorted_links)}"
    ).encode("utf-8")
