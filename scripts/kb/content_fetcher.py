"""B16/B18: 抓取 Flutter 文档 url 网页内容为 markdown + 过期检测。

fetch_content(url) 直接调用 scripts.search.detail.detail(url) 获取 doc 内容
（HTML 已转 Markdown），返回 str。flutter-dev 的 detail(url) 直接接受 url，
不需要 hap-dev 的 object_id+catalog 拆分（Flutter 文档是静态 HTML 站点，无
华为特有 API 路径白名单）。
...
detail() 返回 {'error': ...} 时 raise ValueError（fail-loud，不静默返回空字符串）。
"""
...
def fetch_content(url: str) -> str:
    """抓取 url 对应的 Flutter 文档内容（markdown 字符串）。

    Args:
        url: Flutter 文档 url（如 https://docs.flutter.cn/ui/widgets/layout）。

    Returns:
        doc 的 markdown 内容（str，可能为空——某些 doc 内容为空但抓取成功）。

    Raises:
        ValueError: url 为空 或 detail() 返回 error。
    """
    if not url:
        raise ValueError("fetch_content: url 不能为空")
    result = detail(url)
